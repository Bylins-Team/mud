...
#include "utils/message_capture.h"
...
void SendMsgToChar(const char *msg, const CharData *ch) {
	// Support for MessageCapture (used by identify-export tool)
	MessageCapture* capture = MessageCapture::GetCurrent();
	if (capture && msg) {
		capture->AppendMessage(msg);
		return;
	}
